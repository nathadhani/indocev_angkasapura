...
class Summary_by_year extends Bks_Controller {

    function __construct() 
    {
        $config = array('modules' => 'summary', 'jsfiles' => array('summary_by_year'));
    ...
    function index(){
        ...
        $this->template->title('Transaction by Year');
        $this->template->set('tsmall', 'Summary');
        $this->template->set('icon', 'fa fa-edit');
        $data['auth'] = $this->auth;
        $this->template->build('summary/summary_by_year_v', $data);
    }
    ...
    function excel(){
        ...
        // title column
        $this->excel->setActiveSheetIndex(0)->setCellValue('A1', "Transaction Period " . $tahun);
