...
function encryptCriptoJS(text, base64key) {
    var encryptedBase64Key = base64key;
    var parsedBase64Key = CryptoJS.enc.Base64.parse(encryptedBase64Key);
    var encryptedData = CryptoJS.AES.encrypt(text, parsedBase64Key,{mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.Pkcs7});
    return encryptedData;
}

function decryptCriptoJS(text, base64key) {
    var encryptedBase64Key = base64key;
    var parsedBase64Key = CryptoJS.enc.Base64.parse(encryptedBase64Key);
    var decryptedData = CryptoJS.AES.decrypt(text, parsedBase64Key,{mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.Pkcs7});
    var decryptedText = decryptedData.toString( CryptoJS.enc.Utf8 );
    return decryptedText;
}

function api_ap1_trx_reccord(store_id, id_tr_header){
    if(store_id !== '' && store_id !== null){
        var StringLog = '';
        $.ajax({
            url: baseUrl + 'api/api_ap1/ap1_sha1',
            type: 'POST',
            data: {'method' : 'trxrecord', 'id' : store_id},
            datatype: 'json',
            success: function(tmp) {
                if(tmp.length > 0) {
                    ...
                    $.ajax({
                        url: baseUrl + 'api/api_ap1/ap1_encrypt_trx_reccord',
                        type: 'POST',
                        data: {'id' : id_tr_header, 'sign' : encodeBase64_sha1},
                        datatype: 'json',
                        success: function(data) {
                            if(data.length > 0) {
                                ...
                                $.ajax({
                                    ...
                                    data: {'method' : 'trxrecord', 'dataenc' : "'" + dataenc + "'", 'id' : id_tr_header},
                                    datatype: 'json',
                                    success: function(resp) {
                                        // StringLog = decryptCriptoJS(resp, base64key);
                                        // StringtoFile(StringLog, 'Trx_Record');                  
                                        if (resp) {
                                            let result;
                                            try {
                                                result = JSON.parse(decryptCriptoJS(resp, base64key));                                          
                                                $.ajax({
                                                    url: baseUrl + 'api/api_ap1/ap1_api_getresult',
                                                    type: 'POST',           
                                                    beforeSend: function(){
                                                        $(".ajax-loader").height($(document).height());
                                                        $('.ajax-loader').css("visibility", "visible");
                                                    },                         
                                                    data: {'result' : JSON.stringify(result), 'id' : id_tr_header, 'method' : 'trxadvice'},
                                                    datatype: 'json',
                                                    success: function() {        
                                                        api_ap1_trx_advice(store_id, id_tr_header);                                                  
                                                    },
                                                    error: function(){
                                                        $('.ajax-loader').css("visibility", "hidden");
                                                        alertify.error("can't get result transaction API.!");
        ...
        alertify.error("id store IS NULL");
    }
}

function api_ap1_trx_advice(store_id, id_tr_header){
    if(store_id !== '' && store_id !== null){
        var StringLog = '';
        $.ajax({
            url: baseUrl + 'api/api_ap1/ap1_sha1',
            type: 'POST',
            data: {'method' : 'trxadvice', 'id' : store_id},
            datatype: 'json',
            success: function(tmp) {
                if(tmp.length > 0) {
                    var datasha = JSON.parse(tmp);
                    ...
                    var base64key = encodeBase64(datasha.base64key);
                    $.ajax({
                        url: baseUrl + 'api/api_ap1/ap1_encrypt_trx_advice',
                        type: 'POST',
                        data: {'id' : id_tr_header, 'sign' : encodeBase64_sha1},
                        datatype: 'json',
                        success: function(data) {
                            if(data.length > 0) {
                                ...
                                $.ajax({
                                    ...
                                    data: {'method' : 'trxadvice', 'dataenc' : "'" + dataenc + "'", 'id' : id_tr_header},
                                    datatype: 'json',
                                    success: function(resp) {
                                        StringLog = decryptCriptoJS(resp, base64key);
                                        // StringtoFile(StringLog, 'Trx_Record');
...
function api_ap1_trx_refund(store_id, id_tr_header){
    if(store_id !== '' && store_id !== null){
        var StringLog = '';
        $.ajax({
            url: baseUrl + 'api/api_ap1/ap1_sha1',
            type: 'POST',
            data: {'method' : 'refundtrx', 'id' : store_id},
            datatype: 'json',
            success: function(tmp) {
                if(tmp.length > 0) {
                    var datasha = JSON.parse(tmp);
                    ...
                    var base64key = encodeBase64(datasha.base64key);
                    $.ajax({
                        url: baseUrl + 'api/api_ap1/ap1_encrypt_trx_refund',
                        type: 'POST',
                        data: {'id' : id_tr_header, 'sign' : encodeBase64_sha1},
                        datatype: 'json',
                        success: function(data) {
                            if(data.length > 0) {
                                ...
                                $.ajax({
                                    ...
                                    data: {'method' : 'refundtrx', 'dataenc' : "'" + dataenc + "'", 'id' : id_tr_header},
                                    datatype: 'json',
                                    success: function(resp) {
                                        StringLog = decryptCriptoJS(resp, base64key);
                                        // StringtoFile(StringLog, 'Trx_Record');
                                        ...
                                        if (resp) {
                                            ...
                                            try {
                                                ...
                                                $.ajax({
                                                    ...
                                                    data: {'result' : JSON.stringify(result), 'id' : id_tr_header, 'method' : 'refundtrx'},
                                                    datatype: 'json',
                                                    success: function() {                                                                                                                
                                                        back_to_page_show(id_tr_header);
                                                    },
...
function api_ap2_trx_reccord(id_tr_header){
...
function api_ap2_trx_refund(id_tr_header){
    if(id_tr_header !== '' && id_tr_header !== null){
        $.ajax({
            url: baseUrl + 'api/api_ap2/ap2_post_api',
            type: 'POST',
