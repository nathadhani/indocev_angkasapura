(function ($) {    
    ...
    $("#btn-excel").on('click', function (e) {
        ...
        if($('#company_id').val() === null || $('#company_id').val() === ''){
            ...
            alertify.confirm("download trx period " + $("#periode").val() + " ?", function (e) {    
                if (e) {
                    var url = "summary/summary_by_year/excel/"+$('#periode').val()+"/"+$('#company_id').val()+"/"+$('#store_id').val();
                    $.ajax({
                        url: url,
                        type: 'POST',
                        data: {},
    ...
    function fethdata(){
        ...
        var t = $('#mainTable table').DataTable({
            ...
            sDom: 'it<"row"lp>',
            ajax: {
                url: baseUrl + 'summary/summary_by_year/getData',
                type: 'POST',
                // beforeSend: function(){
                //     $(".ajax-loader").height($(document).height());
                //     $('.ajax-loader').css("visibility", "visible");
                ...
                complete: function(){
                    // $('.ajax-loader').css("visibility", "hidden");
                    $.ajax({
                        url: baseUrl + 'summary/summary_by_year/gettotal',
                        type: 'POST',
                    ...
                    $.ajax({
                        url: baseUrl + 'summary/summary_by_year/getcount',
                        type: 'POST',
                        data: {'company_id' : $("#company_id").val(),
                               'store_id' : $("#store_id").val(),
                               'periode' : $('#periode').val()},
