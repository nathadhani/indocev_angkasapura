(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master_data/m_customer_type/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('master_data/m_customer_type/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#customer_type_name").val(d.customer_type_name).focus();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'master_data/m_customer_type/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTBks(t, 'master_data/m_customer_type/delete');
