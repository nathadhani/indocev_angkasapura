(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#userlevelname").val() == '') {
                bksfn.errMsg("Please fill form");
                return false;
            }
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('user/Level/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('user/Level/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#userlevelname").val(d.userlevelname).focus();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'user/Level/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTBks(t, 'user/Level/delete');
