(function ($) {
    ...
    $("#mainForm").submit(function (e) {
        ...
        $("#ftitle").html('Add');
        $("#nationality_code").html('').focus();
        $("#nationality_desc").html('');
        $("#status").iCheck('check');
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master_data/m_nationality/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('master_data/m_nationality/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#nationality_code").val(d.nationality_code).focus();
        $("#nationality_desc").val(d.nationality_desc);
    ...
    $('#mainTable').selectDTBks(t, 'master_data/m_nationality/delete');
